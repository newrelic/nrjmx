...
#
#
# Upload FIPS RPM artifacts to GH Release assets
#
#
...
# Only upload FIPS RPM files
for filename in $(find . -name "*fips*.rpm");do
  echo "===> Uploading FIPS RPM to GH $TAG: ${filename}"
  while [ "${ATTEMPTS}" -gt 0 ];do
    gh release upload "${TAG}" "${filename}" --clobber
    if [[ "${?}" -eq 0 ]];then
      echo "===> uploaded FIPS RPM ${filename}"
      break
    fi
    set -e
...
echo "=== [upload_fips_rpm] FIPS RPM upload complete"
