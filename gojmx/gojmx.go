...
// checkNRJMXProcessError will check if the nrjmx subprocess returned any error.
func (c *Client) checkNRJMXProcessError() error {
	if c.nrJMXProcess == nil {
...
func (c *Client) QueryMBeanNames(mBeanGlobPattern string) ([]string, error) {
	if err := c.checkNRJMXProcessError(); err != nil {
		return nil, err
	}
