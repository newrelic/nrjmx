...
//
	...
	result, err := client.QueryMbean("test:type=Cat,*")
	assert.NoError(t, err)
...
func Test_Query_Success(t *testing.T) {
	...
	actual, err := client.QueryMbean("test:type=Cat,*")
	assert.NoError(t, err)
...
func Test_WrongMbeanFormat(t *testing.T) {
	...
	actual, err := client.QueryMbean("wrong_format")
	assert.Nil(t, actual)
...
func Test_Connector_Success(t *testing.T) {
	...
	// AND Query returns expected data
	actual, err := client.QueryMbean("jboss.as:subsystem=remoting,configuration=endpoint")
	assert.NoError(t, err)
