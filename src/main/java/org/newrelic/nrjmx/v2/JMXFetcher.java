...
public class JMXFetcher {
    ...
    public boolean connect() throws JMXConnectionError {
        try {
            ...
            String message = String.format("Can't connect to JMX server: '%s', error: '%s', trace: '%s'",
                    connectionString, e.getMessage(), sw.toString());
    ...
    private void queryAttributes(ObjectInstance instance) throws QueryError {
        ...
        for (MBeanAttributeInfo attr : attrInfo) {
            ...
            try {
                ...
                logger.Warn("Can't get attribute " + attrName + " for bean " +
                objectName.toString() + ": "
                + e.getMessage());
                continue;
            }
    ...
    private void parseValue(String name, Object value) throws ValueError {
        ...
        if (value == null) {
            ...
            attrValue.doubleValue = ((Number) value).doubleValue();
            attrValue.valueType = ValueType.DOUBLE;
            result.add(new JMXAttribute(name, attrValue));
            ...
            logger.Info("HashMaps are not supported yet: " + name);
        } else if (value instanceof ArrayList || value.getClass().isArray()) {
            // TODO: Process arrays
            logger.Info("Arrays are not supported yet: " + name);
        } else {
